...
- (void)handlePan:(UIPanGestureRecognizer *)pan {
    ...
    CGFloat minHeight = floor(screenHeight * 0.40);
    CGFloat maxHeight = floor(screenHeight * 0.70);

    if (pan.state == UIGestureRecognizerStateBegan) {
        self.panStartCenter = CGPointMake(CGRectGetMidX(frame), CGRectGetMidY(frame));
        return;
    }

    if (pan.state == UIGestureRecognizerStateChanged) {
        if (translation.y < 0.0) {
            CGFloat height = MIN(maxHeight, CGRectGetHeight(frame) - translation.y);
            frame.size.height = height;
            frame.origin.y = screenHeight - height;
            self.view.frame = frame;
        } else if (translation.y > 0.0) {
            CGFloat height = MAX(1.0, CGRectGetHeight(frame) - translation.y);
            frame.size.height = height;
            frame.origin.y = screenHeight - height;
            self.view.frame = frame;
        }
    ...
    if (pan.state == UIGestureRecognizerStateEnded ||
        ...
        CGFloat targetHeight = minHeight;
        if (translation.y < -80.0)
            targetHeight = maxHeight;

        CGRect target = frame;
        target.size.height = targetHeight;
        target.origin.y = screenHeight - targetHeight;
