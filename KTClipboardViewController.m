...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    if (!self.presentingViewController && self.input) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.15 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (self.input && [self.input respondsToSelector:@selector(becomeFirstResponder)])
                [(UIResponder *)self.input becomeFirstResponder];
        });
    }
}
