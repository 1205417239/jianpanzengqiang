...
/*
 * UIResponder 本身的公开头文件没有声明
 * setInputAccessoryView:，这里补充声明，
 * 避免 Theos 编译时出现 selector 不可见错误。
 */
...
- (void)undo {
    if ([self.input respondsToSelector:@selector(undoManager)]) {
        NSUndoManager *manager = self.input.undoManager;
...
static void KTInstallAccessory(id object) {
    ...
    /*
     保留 App 原本自己的 inputAccessoryView。
     */
...
- (void)setInputAccessoryView:(UIView *)view {
    ...
    /*
     先允许 App 设置自己的 accessory。
     */
    %orig(view);

    /*
     如果输入框已经成为第一响应者，
     下一轮 RunLoop 再把我们的工具栏装上去。
     */
