...
static void KTClipboardOpen(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.35 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            CGFloat height = MIN(620.0, CGRectGetHeight(screenBounds) * 0.74);
            height = MAX(500.0, height);
            if (height > CGRectGetHeight(screenBounds) - 20.0)
                height = CGRectGetHeight(screenBounds) - 20.0;

            UIWindow *window = [[UIWindow alloc] initWithWindowScene:scene];
            window.frame = CGRectMake(0.0,
                                      CGRectGetHeight(screenBounds) - height,
